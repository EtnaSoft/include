...
app.controller('Discovery2',  function( $rootScope, $scope,$http,$timeout, $filter, DiscoveryList,ServiceUpload,ADMIN) {
  ...
  $scope.init = function(postValue){
      ...
      $scope.$watch('postData.typeSearch', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.type', function() {
              $('#sbmtSearch').focus();
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.typeSub', function() {
              $('#sbmtSearch').focus();
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.data', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.filter.attivita', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.where.id', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.where.type', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      $scope.$watch('postData.filter', function() {
              $scope.list.nextPage(0,$scope.postData);
      }, true);
      ...
      $scope.preview = function(position) {
            ...
            /*$rootScope.dialog.open(null,'view','photo','','');
            $http({method: "GET", url: urlJSON+"Photo.php?typePage="+$scope.postData.where.type+"&idPage="+$scope.postData.where.id+"&type="+$scope.postData.type+"&UserId="+$scope.postData.UserId+"&orderBy="+$scope.postData.orderBy+"&limitItemList="+$scope.postData.limitItemList+"&position="+position })
              .success(function(response){
                console.log(urlJSON+"Photo.php?typePage="+$scope.postData.where.type+"&idPage="+$scope.postData.where.id+"&type="+$scope.postData.type+"&UserId="+$scope.postData.UserId+"&orderBy="+$scope.postData.orderBy+"&limitItemList="+$scope.postData.limitItemList+"&position="+position);
                $rootScope.dialog.open(response,'view','photo','','');  console.log(response);
             })
            .error(function(){
               //alert("errore");
            });  */
...
app.factory('DiscoveryList', function($rootScope, $http, $q, ADMIN) {
  ...
  DiscoveryList.prototype.nextPage = function(start,postData) {
        ...
        if(start==0){  this.items=[]; this.postData=postData; this.position=0; this.postData.nearComune=false; this.page=0; this.after=0;  }
