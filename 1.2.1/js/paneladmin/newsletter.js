...
app.controller('AdminNewsletter', function($rootScope, $scope, $http, ListAdmin) {
    ...
    $scope.getMailStrutture = function(){
        $http({method: "POST", url: urlFUNCTION+"paneladmin/controller.php?action=getNewsletterStrutture",data:null , headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
            .success(function(response){ if(response.error==0){
                $scope.strutture=response.data;
            }   $scope.busy=false;  });
    }
    $scope.setting = $scope.getSetting();
    $scope.getMailStrutture();
