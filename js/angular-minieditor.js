...
(function(){
  ...
  var defaultMenus = [
    ...
    ['insertunorderedlist', 'insertorderedlist','createlink', 'insertimage'],
    /*['indent', 'outdent'], */
  ];
  ...
  angular.module('jerryhsia.minieditor', []).directive('minieditor', function($compile, $timeout, minieditorUI) {
      ...
      for (var i = 0; i < menus.length; i++) {
        ...
        for (var j = 0; j < menus[i].length; j++) {
          if (angular.isDefined(miniCommands[menus[i][j]])) {
            ...
            console.log('Comando sconosciuto: ' + menus[i][j]);
          }
        }
        menuText += '</div>';
      }
    ...
    function getMenuItem(commandObj) {
      switch (commandObj.type) {
        case 'button':
          return '<button ng-click="exec(\'' + commandObj.command + '\')" ng-class="{active: states[\''+commandObj.command+'\']}" type="button" class="ic ic-'+commandObj.icon+'-b" title="'+commandObj.title+'" unselectable="on"></button>';
          break;
        default:
          return '';
          break;
